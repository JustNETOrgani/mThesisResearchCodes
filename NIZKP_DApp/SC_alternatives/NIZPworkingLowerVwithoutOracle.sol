...
contract NIZkProofs {
    ...
    bytes32[] PubKeyPool;
    uint256 result = 11; // Random number to be supplied by Provable oraclize.
    ...
    function PubKeyPoolPos(uint256 _UserpubKeyP,uint256 _responseS ) internal view returns (uint256, uint256, uint256){
        uint256 MaxNum      = PubKeyPool.length;
        uint256 posOne      = uint256(result % MaxNum);
